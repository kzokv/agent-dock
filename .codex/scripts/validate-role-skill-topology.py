...
def parse_canonical_roles() -> tuple[dict[str, Path], list[str]]:
    ...
    for path in sorted(AGENTS_DIR.glob("role-*.md")):
        ...
        roles[role_id] = path
    if len(roles) == 0:
        errors.append("no canonical role files found (.codex/agents/role-*.md)")
    return roles, errors
...
def validate_role_files_point_to_matrix(roles: dict[str, Path]) -> list[str]:
    ...
    for role_id, path in roles.items():
        ...
        if "~/.codex/agents/skills-matrix.md" not in text and ".codex/agents/skills-matrix.md" not in text:
            errors.append(f"{path}: Skill Binding section must point to .codex/agents/skills-matrix.md")
