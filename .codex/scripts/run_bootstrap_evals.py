...
SCENARIOS = [
    {
        ...
        "prompt": (
            ...
            "that runs .codex/scripts/bootstrap-budget.sh against tw-portfolio and fails if bootstrap exceeds "
    ...
    },
    {
        ...
        "checks": [
            ...
            ("references changed files", re.compile(r"scripts/onboarding\.sh|\.codex/scripts/bootstrap-budget\.sh|\.codex/AGENTS\.md", re.I)),
...
def main() -> int:
    ...
    try:
        ...
        shared_diff = subprocess.check_output(
            ["git", "-C", str(shared_repo), "diff", "--", ".codex/AGENTS.md", "scripts/onboarding.sh", ".codex/scripts/bootstrap-budget.sh"],
            text=True,
        ).strip()
...
def prepare_eval_home(shared_snapshot: Path, actual_shared_repo: Path, home_dir: Path) -> Path:
    ...
    if codex_home.exists():
        codex_home.unlink()
    os.symlink(shared_snapshot / ".codex", codex_home, target_is_directory=True)
    ...
    for filename in ("config.toml", "config.local.toml"):
        source = actual_shared_repo / ".codex" / filename
        target = shared_snapshot / ".codex" / filename
        if source.exists() and not target.exists():
            shutil.copy2(source, target)
    ...
    if skills_dir.exists() or skills_dir.is_symlink():
        skills_dir.unlink() if skills_dir.is_symlink() else shutil.rmtree(skills_dir)
    os.symlink(codex_home / "skills", skills_dir, target_is_directory=True)
...
def run_retrieval_trials(shared_snapshot: Path, target_snapshot: Path, output_dir: Path) -> list[dict]:
    ...
    trials.append(
        run_retrieval_trial(
            ...
            expected_handles={".codex/AGENTS.md"},
            output_dir=output_dir,
        )
    )
    ...
    trials.append(
        run_retrieval_trial(
            ...
            expected_handles={".codex/AGENTS.md", ".worklog/current-focus.md"},
            output_dir=output_dir,
        )
    )
