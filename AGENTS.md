...
- For broad repo questions that would require opening multiple policy, worklog, prompt, or doc files, prefer the on-demand retrieval flow in `prompts/retrieve.md`; refresh manually with `python3 ~/.codex/scripts/rlm_retrieval.py build --repo <repo> --force` when the catalog is stale and current repo changes matter.
...
- User-level discovery lives at `$HOME/.agents/skills`, which is a curated enabled subset of `~/.codex/agents/skills`.

## On-Demand References

- Knowledge capture and handoff flow: `prompts/curate.md`, `prompts/promote.md`, `prompts/handoff.md`
- On-demand repo retrieval flow: `prompts/retrieve.md`
