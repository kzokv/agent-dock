...
Quality owner for risk-based validation strategy and merge gates.
...
- Sync with `role-product-delivery-manager` on acceptance testability.
- Sync with `role-staff-frontend` and `role-backend-data-engineer` for deterministic hooks.
- Sync with `role-platform-automation` on CI reliability.
...
- Primary: GitHub checks and required status gates.
